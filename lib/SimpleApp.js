...
function SimpleApp (opts, cb) {
  ...
  // TODO: Throw an exception if userDbPath
   ...
   * @type {string}
   */
  self.cookieSecret = opts && opts.cookieSecret
...
SimpleApp.prototype.start = function (cb) {
  ...
  self.server.listen(self.port, function (err) {
    if (!err) debug('SimpleApp started on port ' + self.port)
    cb && cb(err)
  })
}
...
SimpleApp.prototype.close = function (done) {
  ...
  self.server.close(function (err) {
    if (!err) debug('Site closed on port ' + self.port)
    done && done(err)
  })
}